...
@section('login-body')
    ...
    <form method="POST" action="{{ route('login::register') }}" class="text-start">
        ...
        <div id="regular-account-creation" {{ empty(old('create_without_ut_account')) ? "hidden" : "" }}>
            <hr>
            <input type="text" class="form-control" id="calling_name" name="calling_name" placeholder="Calling name" value="{{ old('calling_name') }}">
...
<script type="text/javascript">
    let additionalNameInputs = document.getElementById('regular-account-creation');
    ...
    toggleInput.addEventListener("change", function (event) { 
        ...
        additionalNameInputs.querySelectorAll('input').forEach(function (input) {
            input.required = !createWithUT;
        });

        if (createWithUT) {
            toggleButton.textContent = 'I do not have a University of Twente account';
        } else {
            toggleButton.textContent = 'I have a University of Twente account';
        }
    });
</script>
@endpush
