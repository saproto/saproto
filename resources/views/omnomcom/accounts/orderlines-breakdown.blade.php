            ...
            <div class="card mb-3">
                ...
                <table class="table table-hover table-sm">
                    ...
                    @foreach($accounts as $account_id=>$dates)
                        @php
                            $accountTotal = number_format($dates->sum(function ($date) {
                        ...
                        <tr class="cursor-pointer" data-bs-toggle="collapse"
                            ...
                            <th>{{ $account_id }}</th>
                            <td>{{ $dates->first()->first()->account_name }}</td>
                            <td></td>
                            <td>&euro; {{ $accountTotal }}</td>
                        </tr>

                        @foreach($dates as $date=>$products)
                            <tr class="cursor-pointer collapse collapse-{{ $account_id }}" data-bs-toggle="collapse"
                                data-bs-target=".innercollapse-{{ $account_id }}-{{ $date }}">
                                <td></td>
                                <td>{{ $date }}</td>
                                <td></td>
                                <td>&euro; {{ number_format($products->sum('total'), 2) }}</td>
                            </tr>
