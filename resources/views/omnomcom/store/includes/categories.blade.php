<div class="vh-100 bg-dark col-2 p-0">
    <nav id="category-nav" class="nav p-3">
        @foreach($categories as $category)
            <div class="btn btn-lg btn-category btn-block bg-omnomcom rounded-0 px-2 py-2 text-start {{ ($category == $categories[0] ? 'active' : '') }}"
        ...
        @if(count($minors) > 0)
            <div class="btn btn-lg btn-category btn-block bg-omnomcom rounded-0 px-2 py-2 text-start" data-id="static-minors">
                <strong>{{ count($minors) }}</strong> Minor Members
            </div>
        @endif

        @if(Auth::check())
            <a id="logout-button" href="{{ route('login::logout::redirect', ['route' => 'omnomcom::store::show']) }}"
