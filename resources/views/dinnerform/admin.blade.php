...
@section('container')
    <div class="card mb-3">
        ...
        <div class="card-header bg-dark text-white mb-1">
            Dinnerform orderline overview
            @if($dinnerform->isCurrent())
                <a class="btn btn-warning float-end" href="{{ route('dinnerform::close', ['id' => $dinnerform->id]) }}">
                    close dinnerform!
                </a>
            @elseif(!$dinnerform->closed)
                @include('website.layouts.macros.confirm-modal', [
                                    'action' => route("dinnerform::process", ['id' => $dinnerform->id]),
                                    'text' => 'Process dinnerform',
                                    ...
                                    'confirm' => 'Process',
                                    'classes'=>"btn btn-danger float-end"
                                ])
            @else
                <a class="btn btn-info float-end">
                    This dinnerform is closed and processed!
                </a>
            @endif
        </div>
        ...
        <table class="table table-sm">
                ...
                <td class="align-middle">
                    €{{$dinnerform->totalAmount()}}
                </td>
                <td class="align-middle">
                    {{$dinnerform->amountOfOrders()}}
                </td>
                <td class="align-middle">
                ...
                @if(count($orderList) > 0)
                    @foreach($orderList as $order)
                        <tr>
                            ...
                            <td class="align-middle">{{ $order->user()->name}}<span
                                        class="text-muted"> {{$order->user()->id}}</span></td>
                            <td class="align-middle">
                                {{$order->description}}
                            </td>
