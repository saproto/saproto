    ...
    <form method="post" action="{{ route('dinnerform::orderline::add', ['id'=>$dinnerform->id]) }}">
        ...
        <div class="card mb-3">
            ...
            <div class="card-header bg-dark text-white">
                ...
                @if($dinnerform->event)
                    <br><span
                            class="text-muted"><i><small>For the event: {{$dinnerform->event->title}}</small></i></span>
                @endif
            </div>
                ...
                @else
                    ...
                    <div class="form-group">
                        @if($dinnerform->event->activity && $dinnerform->event->activity->isHelping(Auth::user()))
                            <i><p class="card-text bg-info text-light text-center rounded">
                                    You are registered as a helper on this event and will receive the helper
                                    discount of {{$dinnerform->discount}}% off your order!
                                </p></i>
                        @else
                            <span
                                    class="text-muted"><i>You are not registered as a helper on this event.</i></span>
                        @endif
                    </div>
                @endif
            </div>
