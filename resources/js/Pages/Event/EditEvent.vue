...
const defaultFormData = computed(() => {
  return props.event
    ? ({
        ...
        secret: props.event?.secret,
        committee_id: props.event?.committee_id,
      } as Partial<App.Models.Event>)
    : ({
        title: '',
        location: '',
        maps_location: '',
...
const form = useForm('Event', defaultFormData.value);

const postForm = () => {
  if (form.isDirty) {
    form.post(route('event::add'), {
      preserveScroll: true,
      onSuccess: () => {
          ...
          <InputGroup name="start">
            ...
            <template #input>
              <DateTimeInput v-model="form.start" type="datetime-local" name="start" before-hover>
                <template #before>
                  <button class="p-2 fas fa-calendar-day"></button>
                </template>
              </DateTimeInput>
            </template>
          </InputGroup>
