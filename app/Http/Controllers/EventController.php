...
class EventController extends Controller
{
    ...
    public function apiUpcomingEvents($limit, Request $request)
    {
        ...
        foreach ($events as $event) {
            ...
            $data[] = (object)[
                ...
                'total_places' => ($event->activity ? $event->activity->spots : null),
    ...
    public function icalCalendar($personal_key = null)
    {
        ...
        foreach (Event::where('start', '>', strtotime('-6 months'))->get() as $event) {
            ...
            } elseif ($event->activity !== null && $event->activity->spots == -1) {
                $info_text = 'Sign-up required, but no participant limit.';
            } elseif ($event->activity !== null && $event->activity->spots > 0) {
                $info_text = 'Sign-up required! There are roughly ' . $event->activity->freeSpots() . ' of ' . $event->activity->spots . ' places left.';
